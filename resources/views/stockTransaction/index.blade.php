<x-layout>
    ...
    <div class="w-full pb-2" x-data="{ open: false }">
        ...
        <div x-show="open" x-cloak x-transition class="border p-6 rounded-lg bg-gray-50 shadow">
            ...
            <form action="{{ route('stockTransaction.store') }}" method="POST" class="space-y-4"
                ...
                <div>
                    <label for="notes" class="w-full block text-sm font-medium text-gray-700">Notes</label>
                    <textarea type="text" name="notes" id="notes" required
                        ...
                        <tr
                            ...
                            <td class="px-6 py-4">
                                ...
                                @if (Auth::user()->role === 'staff')
                                    @if ($stockTransaction->status === 'pending')
                                        <div class="flex space-x-2">
                                            ...
                                            <a href="{{ route('stockTransaction.edit', $stockTransaction->id) }}"
                                                class="button text-white bg-blue-700 hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 font-medium rounded-full text-sm px-5 py-2.5 text-center me-2 mb-2 dark:bg">Edit</a>
                                            {{-- Form delete --}}
                                            <form
                                                action="{{ route('stockTransaction.destroy', $stockTransaction->id) }}"
                                    ...
                                    @else
                                        <span
                                            class="text-white bg-gray-400 cursor-not-allowed font-medium rounded-full text-sm px-5 py-2.5 text-center me-2 mb-2">
                                            Edit
                                        </span>
                                    @endif
                                @endif
                                ...
                                @if (Auth::user()->role === 'manajer')
                                    @if ($stockTransaction->status === 'pending')
                                        <form action="{{ route('stockTransaction.approve', $stockTransaction->id) }}"
                                            method="POST" class="inline button"
                                            onsubmit="return confirm('Are you sure accept this transaction ?')">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit"
                                                class="text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-4 focus:ring-green-300 font-medium rounded-full text-sm px-5 py-2.5 text-center me-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-900">Accept</button>
                                        </form>
                                        <form action="{{ route('stockTransaction.reject', $stockTransaction->id) }}"
                                            method="POST" class="inline button"
                                            onsubmit="return confirm('Are you sure reject this transaction ?')">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit"
                                                class="text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-4 focus:ring-red-300 font-medium rounded-full text-sm px-5 py-2.5 text-center me-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">Reject</button>
                                        </form>
                                    @else
                                ...
                                @if (Auth::user()->role === 'admin')
                                    ...
                                    {{-- Form delete --}}
                                    <form action="{{ route('stockTransaction.destroy', $stockTransaction->id) }}"
                                        method="POST" class="inline button"
                                        onsubmit="return confirm('Yakin hapus transaction ini?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                            class="text-white mt-3 bg-red-700 hover:bg-red-800 focus:outline-none focus:ring-4 focus:ring-red-300 font-medium rounded-full text-sm px-5 py-2.5 text-center me-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900">Hapus</button>
                                    </form>
                                @endif
